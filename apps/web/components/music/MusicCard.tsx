...
export function MusicCard({ music, index }: MusicCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-2xl bg-white/80 backdrop-blur-sm p-6 border border-border/50 floating floating-hover transition-all duration-300"
    >
      {imageUrl && (
      ...
      <div>
        <h3 className="text-xl font-serif font-medium mb-1">{music.title}</h3>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {music.streamingLinks.map((link, idx) => (
              <a
                ...
                className="flex items-center gap-1 rounded-full bg-primary/10 px-4 py-2 text-sm text-primary transition-all hover:bg-primary hover:text-primary-foreground hover:scale-105"
              >
                {link.platform}
